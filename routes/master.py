...
@router.get("/master")
def master_page(
    request: Request, tag_code: str | None = None, error: str | None = None
):
    ...
    if role != "Admin":
        raise HTTPException(
            status_code=403, detail="You do not have permission to access Master Data"
        )
    ...
    if tag_code:
        selected_master = (
            ...
            .filter(models.MasterTable.tag_code == tag_code)
            .first()
        )
        ...
        if selected_master:
            values = (
                ...
                .filter(models.MasterListTable.tag_code == tag_code)
                .all()
            )
...
def delete_master(master_id: int):
    ...
    db = SessionLocal()
    master = (
        db.query(models.MasterTable).filter(models.MasterTable.id == master_id).first()
    )
    if master:
...
def add_master_value(tag_code: str, value: str = Form(...)):
    value = value.strip()
    db = SessionLocal()
    master = (
        db.query(models.MasterTable)
    ...
    if master:
        existing_value = (
            ...
            .filter(
                ...
                models.MasterListTable.value == value,
            )
            .first()
        )
        ...
        if not existing_value:
            ...
            return RedirectResponse(
                url=f"/master?tag_code={tag_code}&error=duplicate", status_code=303
            )

    db.close()

    return RedirectResponse(url=f"/master?tag_code={tag_code}", status_code=303)
...
def delete_master_value(value_id: int):
    ...
    master_value = (
        db.query(models.MasterListTable)
        .filter(models.MasterListTable.id == value_id)
        .first()
    )
    if master_value:
