...
def dashboard(
    ...
    if role == "User":
        ...
        if search:
            search = search.strip()
            if search.isdigit():
                query = query.filter(models.Ticket.id == int(search))
            else:
        ...
        if status:
            ...
            ).filter(models.MasterListTable.value == status)
        ...
        in_progress_tickets = sum(
            1 for ticket in tickets if ticket.status.value == "In Progress"
        )
        resolved_tickets = sum(
            1 for ticket in tickets if ticket.status.value == "Resolved"
        )
        closed_tickets = sum(1 for ticket in tickets if ticket.status.value == "Closed")
        db.close()
    ...
    if role in ["Admin", "Support Staff"]:
        ...
        if search:
            ...
            if search.isdigit():
                ...
                query = query.join(
                    models.User, models.Ticket.created_by == models.User.id
                ).filter(
        ...
        support_staff = (
            ...
            .join(
                models.MasterListTable, models.User.role_id == models.MasterListTable.id
            )
            .filter(models.MasterListTable.value == "Support Staff")
            .all()
        )
        ...
        return templates.TemplateResponse(
            name="staff - admin/staff_dashboard.html",
            request=request,
            context={
                "user": user,
                "tickets": tickets,
...
def my_assigned_tickets(
    ...
    if status:
        ...
        query = query.join(
            models.MasterListTable, models.Ticket.status_id == models.MasterListTable.id
        ).filter(models.MasterListTable.value == status)

    # Filter by Priority
    if priority_id:

        query = query.filter(models.Ticket.priority_id == int(priority_id))
    ...
    resolved_tickets = sum(1 for ticket in tickets if ticket.status.value == "Resolved")
